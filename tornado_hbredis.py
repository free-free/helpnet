...
class _PipelineWrapper(object):
	...
	def _pipeline_commands(self,*args,**kw):
		...
		if len(commands)==1:
			...
			self.__pipeline.stack_call(commands[0],commands[1],*list(parsed_args))
		else:
			pass
...
class TornadoHBRedis(
	...
	def __init__(self,host,port,autoconnect=True):
		...
		self._client=tornadis.Client(host=self._host,port=self._port,autoconnect=self._autoconnect)
	def pipeline(self):
		self.__pipeline_wrapper=_PipelineWrapper(tornadis.Pipeline(),self._client)
